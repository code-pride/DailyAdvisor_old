...
function* fetchRandomUser(action) {
    ...
    try {
        yield put({ type: FETCH_RANDOM_USER_STARTED });

        const { data } = yield call(requestRandomUser, params);
        const user = data.results[0];

        yield put({ type: FETCH_RANDOM_USER_SUCCEEDED,  payload: { ...user } });
        yield put({ type: FETCH_RANDOM_USER_FINISHED });
    } catch (error) {
        yield put({ type: FETCH_RANDOM_USER_FAILED, payload: { error } });
    }
}
